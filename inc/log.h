...
#define LOG(LEVEL, MESSAGE, ...) loglvl(__FILE__, __LINE__, LEVEL, MESSAGE , ## __VA_ARGS__)

#if LOG_LEVEL == LOG_DEBUG
#define DEBUG(MESSAGE, ...) LOG(LOG_DEBUG, MESSAGE , ## __VA_ARGS__)
#else
...
#define WARN(MESSAGE, ...) LOG(LOG_WARN,  MESSAGE , ## __VA_ARGS__)
#else
#define WARN(MESSAGE, ...)
#endif 

#if LOG_LEVEL == LOG_DEBUG || LOG_LEVEL == LOG_INFO || LOG_LEVEL == LOG_WARN || LOG_LEVEL == LOG_ERROR
#define ERROR(MESSAGE, ...) LOG(LOG_ERROR, MESSAGE , ## __VA_ARGS__)
#else
#define ERROR(MESSAGE, ...)
#endif

// Fatal errors are always included
#define FATAL(MESSAGE, ...) LOG(LOG_FATAL, MESSAGE , ## __VA_ARGS__)
